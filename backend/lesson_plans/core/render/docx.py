...
Adapted from the M1 spike: streams into an in-memory `BytesIO` instead of writing
to a filesystem path, since the production caller serves the bytes over HTTP
rather than saving a file (design D1a — "adapt render_docx to stream to BytesIO").
"""
