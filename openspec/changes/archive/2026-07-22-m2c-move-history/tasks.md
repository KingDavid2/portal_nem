...
| D4 | RLS backstop test (raw psycopg as portal_app, two-workspace insert with no single-workspace scoped context) + migrate --check clean | test_move.py + config confirmation | `pytest backend/workspaces/tests/test_move.py -k rls` | `manage.py migrate --check` | none (test-only addition) |
