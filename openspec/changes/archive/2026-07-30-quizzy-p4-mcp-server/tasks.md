...
- [x] 3.7 RED same file: `search_catalog(query="", field=None)` returns the whole catalog; a bad `field` id surfaces as `ToolInputError` (from `catalog.field_by_id`'s `KeyError`); matching is `catalog._normalize(needle) in catalog._normalize(haystack)` (`core/catalog.py:153`) over field/subject/theme/content/PDA text; a content matching via any PDA is rendered whole. *(S3b on `feat/quizzy-p4-s3b-search-catalog`.)*
- [x] 3.8 RED same file: `search_catalog` serves a membership in an **empty** workspace — the result depends on no workspace-scoped row. *(S3b.)*
- [x] 3.9 RED same file: no tool creates, updates, or deletes any row — assert across all five with a write-counting hook. *(S3 four tools; S3b extended write-proof to `search_catalog`.)*
- [x] 3.10 GREEN `backend/mcp_server/tools.py`: the five sync tool bodies, each entering `workspace_scope(membership.workspace_id)` itself and reusing the serializers above. No new payload shape. *(S3b replaced the `search_catalog` stub with the real D1 body.)*
