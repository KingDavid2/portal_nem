...
from lesson_plans.render.docx import render_docx
...
def test_datos_table_is_first_and_key_value_shaped(tmp_path):
    out = tmp_path / "p.docx"
    render_docx(_proyecto(), out)
...
def test_headings_and_duracion_present(tmp_path):
    out = tmp_path / "p.docx"
    render_docx(_proyecto(), out)
    doc = Document(str(out))
...
def test_paso_dinamica_table_present(tmp_path):
    out = tmp_path / "p.docx"
    render_docx(_proyecto(), out)
    doc = Document(str(out))
...
def test_rubric_table_has_criterion_and_four_levels(tmp_path):
    out = tmp_path / "p.docx"
    render_docx(_proyecto(), out)
    doc = Document(str(out))
    all_tables = [_rows(t) for t in doc.tables]
